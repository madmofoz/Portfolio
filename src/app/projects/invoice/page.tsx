...
function NotaContent() {
  ...
  // Core Universal States
  const [invoiceNo, setInvoiceNo] = useState("");
  const [date, setDate] = useState("");
  const [status, setStatus] = useState("PAID");
  const [paymentMethod, setPaymentMethod] = useState("Bank Transfer");
  
  const [customer, setCustomer] = useState({ name: "", phone: "", meta: "" });
  const [assignment, setAssignment] = useState({ title: "", code: "", metric: "" });
  const [techSpecs, setTechSpecs] = useState({ category: "", system: "", lead: "Muhammad Zhifrantino" });
  ...
  useEffect(() => {
    if (encodedData) {
      try {
        ...
        // Backward-compatibility fallback if old link format is opened
        setAssignment({
          title: parsed.assignment?.title || parsed.vehicle?.model || "",
          code: parsed.assignment?.code || parsed.vehicle?.plateNo || "",
          metric: parsed.assignment?.metric || parsed.vehicle?.odometer || parsed.vehicle?.battVoltage || ""
        });
        
        setTechSpecs({
          category: parsed.techSpecs?.category || parsed.tuningLog?.acc || "Engineering",
          system: parsed.techSpecs?.system || parsed.tuningLog?.ecuMap || "",
          lead: parsed.techSpecs?.lead || parsed.tuningLog?.mechanic || "Muhammad Zhifrantino"
        });
  ...
  // Generate short link via tinyurl
  const handleGenerateLink = async () => {
    const payload = { invoiceNo, date, status, paymentMethod, customer, assignment, techSpecs, items };
    const jsonString = encodeURIComponent(JSON.stringify(payload));
    const base64Encoded = btoa(jsonString);
    
    const baseUrl = window.location.origin + window.location.pathname;
    const finalLongUrl = `${baseUrl}?d=${base64Encoded}`;
    
    try {
      // Tembak classic API TinyURL yang mengembalikan plain text url pendek
      const response = await fetch(`https://tinyurl.com/api-create.php?url=${encodeURIComponent(finalLongUrl)}`);
      ...
      if (response.ok) {
        ...
        setShareableLink(finalLongUrl);
        navigator.clipboard.writeText(finalLongUrl);
      }
    } catch (error) {
      // Fallback jika ada network error
      setShareableLink(finalLongUrl);
      navigator.clipboard.writeText(finalLongUrl);
    }
        ...
        {showControlPanel && (
          <div className="lg:col-span-5 w-full bg-white dark:bg-zinc-900/50 backdrop-blur-md p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 h-fit space-y-5 print:hidden shadow-sm">
            <h2 className="text-sm font-black tracking-wider text-zinc-900 dark:text-zinc-100 uppercase border-b border-zinc-200 dark:border-zinc-800 pb-2 flex items-center gap-2">
              <span>MADZ</span> Panel
            </h2>
            
            {/* Meta Info */}
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <label className="font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider text-[10px]">No. Invoice</label>
              ...
              <div>
                <label className="font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider text-[10px]">Date</label>
            ...
            <div className="space-y-2 pt-3 border-t border-zinc-200 dark:border-zinc-800 text-xs">
              <h3 className="font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider text-[10px]">Client / Customer</h3>
              <input type="text" placeholder="Client Name / Instance" value={customer.name} onChange={(e) => setCustomer({...customer, name: e.target.value})} className="w-full p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
              <input type="text" placeholder="No. WhatsApp / Contact" value={customer.phone} onChange={(e) => setCustomer({...customer, phone: e.target.value})} className="w-full p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
              <input type="text" placeholder="Additional Info (e.g. Address / Company)" value={customer.meta} onChange={(e) => setCustomer({...customer, meta: e.target.value})} className="w-full p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
            </div>

            {/* Assignment Block */}
            <div className="space-y-2 pt-3 border-t border-zinc-200 dark:border-zinc-800 text-xs">
              <h3 className="font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider text-[10px]">Project / Assignment Details</h3>
              <input type="text" placeholder="Project Name / Unit (e.g. werkstätte tuning / CAD 3D Enclosure)" value={assignment.title} onChange={(e) => setAssignment({...assignment, title: e.target.value})} className="w-full p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
              <div className="grid grid-cols-2 gap-2">
                <input type="text" placeholder="Reg. No / Code (e.g. Plat / Part No)" value={assignment.code} onChange={(e) => setAssignment({...assignment, code: e.target.value})} className="p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
                <input type="text" placeholder="Scope / Tracker (e.g. 14k KM / 3 Assemblies)" value={assignment.metric} onChange={(e) => setAssignment({...assignment, metric: e.target.value})} className="p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
              </div>
            </div>

            {/* Universal System Logs */}
            <div className="space-y-2 pt-3 border-t border-zinc-200 dark:border-zinc-800 text-xs">
              <h3 className="font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider text-[10px]">Technical Specifications</h3>
              <div className="grid grid-cols-3 gap-2">
                <input type="text" placeholder="Category (e.g. CAD Design / Tuning)" value={techSpecs.category} onChange={(e) => setTechSpecs({...techSpecs, category: e.target.value})} className="p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 text-center focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
                <input type="text" placeholder="System/Engine (e.g. SW 2026 / werkstätte tuning)" value={techSpecs.system} onChange={(e) => setTechSpecs({...techSpecs, system: e.target.value})} className="p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 text-center focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
                <input type="text" placeholder="Lead Mech/Designer" value={techSpecs.lead} onChange={(e) => setTechSpecs({...techSpecs, lead: e.target.value})} className="p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded text-zinc-900 dark:text-zinc-100 text-center focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition" />
              </div>
            </div>
        ...
        <div className="lg:col-span-7 w-full bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-800 rounded-xl p-8 shadow-xl print:shadow-none print:border-none print:p-0 print:bg-white print:text-black">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-zinc-200 dark:border-zinc-800 pb-6 mb-6 print:border-zinc-200">
            <div>
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 uppercase tracking-widest font-medium">High-Performance Engineering & Digital Solutions</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-zinc-200 dark:border-zinc-800 pb-6 mb-6 text-sm print:border-zinc-200">
            <div>
              <h3 className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Client / Customer</h3>
              ...
              {customer.meta && <p className="text-zinc-500 dark:text-zinc-500 text-xs mt-0.5">{customer.meta}</p>}
            </div>
            <div>
              <h3 className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Assignment Reference</h3>
              <p className="font-bold text-zinc-900 dark:text-zinc-200 print:text-black">{assignment.title || '-'}</p>
              {assignment.code && <p className="text-zinc-600 dark:text-zinc-400 text-xs font-mono mt-0.5">ID/Reg: {assignment.code}</p>}
              {assignment.metric && <p className="text-zinc-600 dark:text-zinc-400 text-xs font-mono mt-0.5">Scope: {assignment.metric}</p>}
            </div>
            <div className="border border-zinc-200 dark:border-zinc-800 p-3 rounded-lg bg-zinc-50 dark:bg-zinc-950/30 print:bg-transparent print:border-zinc-300">
              <h3 className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5">System Specs Log</h3>
              {techSpecs.category && <p className="text-xs text-zinc-700 dark:text-zinc-300 print:text-zinc-700"><span className="font-bold text-zinc-400 dark:text-zinc-500">Category:</span> {techSpecs.category}</p>}
              {techSpecs.system && <p className="text-xs text-zinc-700 dark:text-zinc-300 print:text-zinc-700 mt-0.5"><span className="font-bold text-zinc-400 dark:text-zinc-500">System/Ver:</span> {techSpecs.system}</p>}
              {techSpecs.lead && <p className="text-xs text-zinc-700 dark:text-zinc-300 print:text-zinc-700 mt-0.5"><span className="font-bold text-zinc-400 dark:text-zinc-500">Lead Operator:</span> {techSpecs.lead}</p>}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start pt-4 border-t border-zinc-200 dark:border-zinc-800 print:border-zinc-300">
            <div className="mb-4 sm:mb-0 text-xs text-zinc-500 dark:text-zinc-400 max-w-sm print:text-zinc-500">
              ...
              <p className="leading-relaxed">Thank you for trusting your systems to madz werkstätte. Warranty is valid under specified terms from the official hand-over date.</p>
            </div>
